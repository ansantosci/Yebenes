...
function renderStructure(){
  ...
  if(dbStructureLoaded){
    ...
    $('#structureTeamsTable').innerHTML=dbSeasonTeams().map(t=>`<tr><td><strong>${esc(t.name)}</strong></td><td>${esc(dbCategoryName(t.categoryId))}</td><td>${t.modality?esc(t.modality):'—'}</td><td>—</td><td>—</td><td><span class="status ${t.active?'complete':'returned'}">${t.active?'Activo':'Inactivo'}</span></td><td><span class="meta">Supabase</span></td></tr>`).join('');
    return;
  }
