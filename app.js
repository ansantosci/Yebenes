...
async function loadRemoteClubData(){
  ...
  remoteClubPlayers=(playerRows||[]).map(p=>{
    ...
    const appointments=(appointmentRows||[]).filter(a=>a.jugador_id===p.id).sort((a,b)=>String(b.fecha_hora||'').localeCompare(String(a.fecha_hora||'')));const appointment=appointments.find(a=>a.estado==='programada'&&new Date(a.fecha_hora)>=new Date())||appointments.find(a=>a.estado==='programada')||null;const appointmentNotifications=appointment?notificationRows.filter(n=>n.cita_id===appointment.id):[];return {id:p.id,personId:p.persona_id,name:[person.nombre,person.primer_apellido,person.segundo_apellido].filter(Boolean).join(' '),birth:person.fecha_nacimiento||'',active:p.activo!==false,inscription:ins,categoryName:ins?dbCategoryName(ins.categoria_id):'—',representation:rep,tutorName:repPerson?[repPerson.nombre,repPerson.primer_apellido,repPerson.segundo_apellido].filter(Boolean).join(' '):'Sin representante',assignment,teamName:team?.name||'Sin equipo',requirements,fedDocs,docsLabel:docsComplete?'Completa':requirements.some(r=>r.estado==='rechazado'||r.estado==='nueva_version_solicitada')?'Revisar':requirements.some(r=>['aportado','declarado_realizado'].includes(r.estado))?'En revisión':'Pendiente',medicals:medicalsForPlayer,medical:latestRemoteMedical,appointments,appointment,appointmentNotifications};
  });
...
async function renderRemoteClub(){
  ...
  try{
    ...
    const rows=remoteClubPlayers.filter(v=>{const statusGroup=remoteStateIsComplete(v.inscription?.estado)?'complete':'pending';const d18=v.birth?dayDiff(isoToday(),addYears(v.birth,18)):null;const quick=clubQuickFilter==='all'||(clubQuickFilter==='complete'&&remoteStateIsComplete(v.inscription?.estado))||(clubQuickFilter==='docs'&&v.docsLabel!=='Completa')||(clubQuickFilter==='review'&&['pendiente_revision_inicial','devuelta_familia'].includes(v.inscription?.estado))||(clubQuickFilter==='medical90'&&medicalState(v.medical?{date:v.medical.fecha_reconocimiento,expiry:v.medical.fecha_valido_hasta}:null).key==='soon')||(clubQuickFilter==='adult30'&&d18!==null&&d18>=0&&d18<=30);return quick&&(f==='all'||statusGroup===f)&&(cf==='all'||v.inscription?.categoria_id===cf)&&(af==='all'||(af==='active')===v.active)&&v.name.toLowerCase().includes(q)});
    $('#adminTable').innerHTML=rows.length?rows.map(v=>`<tr class="admin-row ${!v.active?'inactive-row':''}" data-id="${esc(v.id)}" tabindex="0"><td><strong>${esc(v.name)}</strong>${!v.active?'<div class="meta inactive-note">Jugador inactivo</div>':''}${v.inscription?.estado==='devuelta_familia'?'<div class="meta return-note">Devuelto a familia</div>':''}</td><td>${esc(v.tutorName)}</td><td>${esc(v.categoryName)}</td><td><strong>${esc(v.teamName)}</strong></td><td><span class="status ${v.active?'complete':'returned'}">${v.active?'Activo':'Inactivo'}</span></td><td><span class="dot ok">✓ Completo</span></td><td>${(()=>{const p=federationDocsProgress(v);return `${p.pct}% · ${p.valid}/${p.total}`})()}</td><td><span class="workflow-pill ${esc(v.inscription?.estado||'')}">${esc(remoteStateLabel(v.inscription?.estado))}</span></td></tr>`).join(''):'<tr><td colspan="8">No hay jugadores que coincidan con los filtros.</td></tr>';
