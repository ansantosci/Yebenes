...
async function renderRemoteMedical(){
  ...
  try{
    ...
    const rows=source.filter(x=>{const appt=!!x.appointment;const special=f==='appointment_scheduled'?appt:f==='appointment_pending'?(x.state.key==='soon'&&!appt):null;return(!q||x.name.toLowerCase().includes(q))&&(f==='all'||(special!==null?special:x.state.key===f))});
