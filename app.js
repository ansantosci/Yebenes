...
async function renderRemoteClub(){
  ...
  try{
    ...
    const operational=remoteClubPlayers.filter(v=>v.active);$('#statPlayers').textContent=operational.length;$('#statComplete').textContent=operational.filter(v=>remoteStateIsComplete(v.inscription?.estado)).length;$('#statDocs').textContent=operational.filter(v=>v.docsLabel!=='Completa').length;$('#statReview').textContent=operational.filter(v=>['pendiente_revision_inicial','devuelta_familia'].includes(v.inscription?.estado)).length;$('#statMedical90').textContent='0';$('#statAdult30').textContent=operational.filter(v=>{const d=v.birth?dayDiff(isoToday(),addYears(v.birth,18)):null;return d!==null&&d>=0&&d<=30}).length;
...
async function advanceRemoteSelected(){const v=remoteClubPlayers.find(x=>x.id===selectedRemotePlayerId);if(!v||currentRole==='coach')return;const order=['pendiente_revision_inicial','datos_validados','documentacion_validada','lista_para_federar','ficha_tramitada'];let ix=order.indexOf(v.inscription.estado);if(v.inscription.estado==='devuelta_familia')ix=0;const next=order[Math.min(Math.max(ix,0)+1,order.length-1)];try{await updateRemoteInscriptionState(v,next,`Estado avanzado por ${currentUser.name}`,next==='datos_validados'?'':v.inscription.observaciones);await renderRemoteClub();await openRemoteAdminPlayer(v.id)}catch(err){alert(`No se ha podido avanzar el estado: ${err.message||err}`)}}
async function setRemoteWorkflowSelected(){const v=remoteClubPlayers.find(x=>x.id===selectedRemotePlayerId);if(!v||currentRole==='coach')return;const next=$('#adminWorkflow').value;if(!next)return;try{await updateRemoteInscriptionState(v,next,`Estado cambiado manualmente por ${currentUser.name}`,next==='devuelta_familia'?v.inscription.observaciones||'Revisión solicitada por el club':'');await renderRemoteClub();await openRemoteAdminPlayer(v.id)}catch(err){alert(`No se ha podido guardar el estado: ${err.message||err}`)}}
