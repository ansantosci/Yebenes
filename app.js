...
const K={
  ...
  audit:`yebenes-audit-v${VERSION}`,currentSeason:`yebenes-current-season-v${VERSION}`,sessionRole:`yebenes-session-role-v${VERSION}`
};
...
const seedUsers=[
  ...
  {id:'u-family',name:'Familia Santos',email:'familia@yebenes.demo',phone:'600000000',password:'demo123',roles:['family'],active:true},
...
let users=[],players=[],seasons=[],categories=[],teams=[],inscriptions=[],representations=[],playerTeams=[],medicals=[],coaches=[],statusHistory=[],audit=[],currentUser=null,currentRole=null,currentSeasonId='s-2026';
...
function saveAll(){saveUsers();savePlayers();saveSeasons();saveCategories();saveTeams();saveInscriptions();saveReps();savePlayerTeams();saveMedicals();saveCoaches();saveStatusHistory();saveAudit()}
...
function initData(){
  ...
  players=players.map(normalizePlayer);recalculateSeasonCategories(currentSeasonId);savePlayers();applyAgeTransitions();
}
function reloadCore(){
  ...
  coaches=readJSON(K.coaches,seedCoaches);statusHistory=readJSON(K.statusHistory,[]);audit=readJSON(K.audit,[]);currentSeasonId=readJSON(K.currentSeason,'s-2026');
}
function reload(){reloadCore();const sid=localStorage.getItem(K.session);currentUser=users.find(u=>u.id===sid&&u.active)||null;const savedRole=localStorage.getItem(K.sessionRole),roles=currentUser?availableRoles(currentUser):[];currentRole=currentUser&&roles.includes(savedRole)?savedRole:(roles[0]||null)}
...
function setView(id){['clubView','medicalView','coachesView','structureView','usersView'].forEach(v=>$('#'+v)?.classList.toggle('active',v===id));$$('.club-nav .nav-item').forEach(x=>x.classList.toggle('active',x.dataset.clubView===id))}
...
function refreshAdminTeamSelect(v){const sel=$('#adminTeam');sel.innerHTML='<option value="">Sin equipo asignado</option>'+currentSeasonTeams().filter(t=>t.active&&t.categoryId===v.i.categoryId).map(t=>`<option value="${t.id}">${esc(t.name)}</option>`).join('');sel.value=v.ta?.teamId||'';const can=currentRole!=='coach'&&v.i.workflow!=='review';sel.disabled=!can;$('#saveTeamAssignment').disabled=!can;$('#teamAssignHint').textContent=currentRole==='coach'?'Solo consulta.':v.i.workflow==='review'?'Primero valida la inscripción inicial para poder asignar equipo.':'Los cambios de equipo conservan el histórico.';renderTeamAssignmentHistory(v.p.id)}
function renderTeamAssignmentHistory(playerId){$('#teamAssignmentHistory').innerHTML=playerTeams.filter(a=>a.playerId===playerId).sort((a,b)=>(b.startDate||'').localeCompare(a.startDate||'')).map(a=>`<div class="history-item"><strong>${esc(teamName(a.teamId))}</strong><span>${esc(seasonName(a.seasonId))} · ${fmt(a.startDate)} → ${fmt(a.endDate)}</span></div>`).join('')||'<div class="meta">Sin asignaciones anteriores.</div>'}
function renderRepresentation(v){const p=v.p,r=v.rep,a=ageOn(p.birth),d=daysTo18(p),pu=currentPlayerUser(p);let text='';if(r?.type==='self')text=`<strong>Autorrepresentación</strong><span>${esc(repUserName(r))} · desde ${fmt(r.startDate)}</span>`;else if(r?.type==='guardian')text=`<strong>Tutor activo: ${esc(repUserName(r))}</strong><span>Desde ${fmt(r.startDate)}${r.legalException?' · excepción legal registrada':''}</span>`;else text='<strong>Sin representante activo</strong><span>Revisar situación.</span>';if(d!==null&&d>=0&&d<=30)text+=`<div class="adult-transition">Cumple 18 años en ${d} días. Conviene preparar su cuenta.</div>`;if(a>=18&&!pu&&!r?.legalException)text+='<div class="adult-transition danger">Mayor de edad sin cuenta propia activa.</div>';$('#representationSummary').innerHTML=text;const admin=currentRole==='admin';$('#changeTutorButton').disabled=!admin||r?.type==='self';$('#selfRepresentationButton').disabled=!admin||(!pu&&!(a>=18||(d!==null&&d<=30)));$('#selfRepresentationButton').textContent=pu?(pu.active?'Cuenta de jugador activa':'Cuenta preparada'):'Preparar autorrepresentación'}
function renderPlayerHistory(playerId){const items=[...statusHistory.filter(h=>h.playerId===playerId).map(h=>({date:h.date,title:`Estado: ${workflowLabel(h.from)} → ${workflowLabel(h.to)}`,detail:`${actorName(h.actorId)}${h.note?' · '+h.note:''}`})),...audit.filter(a=>a.playerId===playerId).map(a=>({date:a.date,title:a.action,detail:`${actorName(a.actorId)}${a.detail?' · '+a.detail:''}`}))].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12);$('#playerHistory').innerHTML=items.map(x=>`<div class="history-item"><strong>${esc(x.title)}</strong><span>${new Date(x.date).toLocaleString('es-ES')} · ${esc(x.detail)}</span></div>`).join('')||'<div class="meta">Todavía no hay movimientos registrados.</div>'}
function openAdminPlayer(id){selectedPlayerId=id;const p=visiblePlayers().find(x=>x.id===id);if(!p)return;const v=playerVM(p);$('#adminPlayerName').textContent=p.name;$('#adminPlayerCategory').textContent=`${v.category} · ${seasonName(v.i.seasonId)}`;$('#adminPlayerSource').textContent=v.rep?`Representación actual: ${repUserName(v.rep)}`:'Sin representación activa';$('#adminPlayerData').textContent=p.data?'Completos':'Revisar';$('#adminPlayerDocs').textContent=v.i.docs;$('#adminPlayerFed').textContent=v.i.federation;$('#adminWorkflow').value=v.i.workflow;$('#returnMessage').value=v.i.returnMessage||'';refreshAdminTeamSelect(v);renderRepresentation(v);renderPlayerHistory(id);const readonly=currentRole==='coach',linked=v.rep?.type==='guardian'&&!readonly;$('#adminWorkflow').disabled=readonly;$('#saveWorkflow').disabled=readonly;$('#advanceStatus').disabled=readonly;$('#returnMessage').disabled=!linked;$('#returnToFamily').disabled=!linked;$('#returnHint').textContent=readonly?'Perfil entrenador: acceso de consulta.':linked?'El tutor activo verá el motivo al iniciar sesión.':'No existe un tutor familiar activo al que devolver la ficha.';$('#adminPlayerModal').showModal()}
function updateInscription(playerId,mut,note=''){const i=inscriptionFor(playerId);if(!i)return;const old=i.workflow;Object.assign(i,mut({...i}));saveInscriptions();if(i.workflow!==old)addStatusHistory(i.id,playerId,old,i.workflow,note);recordAudit('Actualización de inscripción',playerId,note||workflowLabel(i.workflow));renderClub()}
...
function openPlayerForNew(){editingPlayerId=null;$('#playerForm').reset();$('#playerModalTitle').textContent='Datos del jugador';$('#savePlayer').textContent='Guardar jugador';refreshCalculatedCategory($('#playerForm'));$('#playerModal').showModal()}
function openPlayerForEdit(id){const p=familyAccessPlayers().find(x=>x.id===id);if(!p)return;editingPlayerId=id;const f=$('#playerForm'),parts=p.name.split(' '),i=inscriptionFor(id);f.elements.name.value=parts[0]||'';f.elements.surname1.value=parts[1]||'';f.elements.surname2.value=parts.slice(2).join(' ');f.elements.birth.value=p.birth||'';f.elements.dni.value=p.dni||'';f.elements.email.value=currentUser.email||'';f.elements.phone.value=currentUser.phone||'';$('#playerModalTitle').textContent='Revisar datos del jugador';$('#savePlayer').textContent='Guardar cambios';refreshCalculatedCategory($('#playerForm'));$('#playerModal').showModal()}
...
$$('input[name="signupMode"]').forEach(r=>r.onchange=()=>{$('#selfSignupFields').classList.toggle('hidden',document.querySelector('input[name="signupMode"]:checked')?.value!=='self')});
$('#loginForm').onsubmit=e=>{e.preventDefault();reload();const fd=new FormData(e.currentTarget),u=users.find(x=>x.email.toLowerCase()===String(fd.get('email')).toLowerCase()&&x.password===fd.get('password')&&x.active);if(!u){alert('Correo, contraseña o estado no válidos.');return}localStorage.setItem(K.session,u.id);currentUser=normalizeUser(u);const avail=availableRoles(currentUser);if(!avail.length){alert(currentUser.pendingActivationDate?`Tu perfil de jugador se activará el ${fmt(currentUser.pendingActivationDate)}.`:'Tu cuenta no tiene perfiles activos.');currentUser=null;localStorage.removeItem(K.session);return}currentRole=avail.length===1?avail[0]:null;recordAudit('Inicio de sesión','',(currentUser.roles||[]).map(roleLabel).join(', '),u.id);e.currentTarget.reset();if(currentRole){localStorage.setItem(K.sessionRole,currentRole);showApp()}else showRoleChooser()};
$('#familySignupForm').onsubmit=e=>{e.preventDefault();reload();const fd=new FormData(e.currentTarget);if(fd.get('password')!==fd.get('password2')){alert('Las contraseñas no coinciden.');return}const email=String(fd.get('email')).toLowerCase();if(users.some(u=>u.email.toLowerCase()===email)){alert('Ya existe una cuenta con ese correo. Inicia sesión: los perfiles adicionales se conceden sobre la misma cuenta.');return}const mode=fd.get('signupMode')||'guardian',name=String(fd.get('name')).trim();if(mode==='self'){const birth=fd.get('birth'),cat=categoryForBirth(birth,currentSeasonId);if(!birth||!cat){alert('La fecha de nacimiento no corresponde a una categoría configurada para la temporada activa.');return}if(ageOn(birth)<18){alert('Los menores de 18 años no pueden registrarse por sí mismos. La inscripción debe realizarla un tutor o representante legal.');return}const pid=uid('p'),iid=uid('i'),u={id:uid('u-person'),name,email,phone:fd.get('phone'),password:fd.get('password'),roles:['player'],active:true,playerId:pid};users.push(u);players.push({id:pid,name,birth,dni:'',data:true,active:true});inscriptions.push({id:iid,playerId:pid,seasonId:currentSeasonId,categoryId:cat.id,docs:'Pendiente de documentos',workflow:'review',federation:'Pendiente de revisión',status:'pending',familyActionRequired:false,returnMessage:'',createdAt:isoToday()});representations.push({id:uid('r'),playerId:pid,userId:u.id,type:'self',startDate:isoToday(),endDate:'',reason:'Autorregistro de jugador adulto',legalException:false});addStatusHistory(iid,pid,'','review','Inscripción creada por jugador adulto');recordAudit('Alta de cuenta de jugador',pid,'Autorregistro adulto',u.id);savePlayers();saveInscriptions();saveReps();saveUsers();currentUser=u;currentRole='player'}else{const u={id:uid('u-person'),name,email,phone:fd.get('phone'),password:fd.get('password'),roles:['family'],active:true};users.push(u);saveUsers();currentUser=u;currentRole='family';recordAudit('Alta de cuenta de tutor','','Autorregistro',u.id)}localStorage.setItem(K.session,currentUser.id);localStorage.setItem(K.sessionRole,currentRole);showApp();e.currentTarget.reset();$('#selfSignupFields').classList.add('hidden')};
$('#logoutButton').onclick=()=>{localStorage.removeItem(K.session);localStorage.removeItem(K.sessionRole);currentUser=null;currentRole=null;showAuth('login')};$('#roleSwitchButton').onclick=()=>showRoleChooser(true);$('#closeRoleChooser').onclick=()=>$('#roleChooserModal').close();
...
$('#addPlayerButton').onclick=openPlayerForNew;$('#playerForm').onsubmit=e=>{e.preventDefault();if(currentRole==='player'&& !editingPlayerId)return;const fd=new FormData(e.currentTarget),full=[fd.get('name'),fd.get('surname1'),fd.get('surname2')].filter(Boolean).join(' '),birth=fd.get('birth'),cat=categoryForBirth(birth,currentSeasonId);if(!cat){alert('La fecha de nacimiento no corresponde a una categoría configurada para la temporada activa.');return}if(editingPlayerId){const p=players.find(x=>x.id===editingPlayerId);if(!p)return;p.name=full;p.birth=birth||'';p.dni=fd.get('dni')||'';p.data=true;const i=inscriptionFor(p.id);if(i){const old=i.workflow;i.categoryId=cat?.id||i.categoryId;i.workflow='review';i.federation='Modificado por usuario · pendiente de revisión';i.status='pending';i.familyActionRequired=false;i.returnMessage='';addStatusHistory(i.id,p.id,old,'review','Datos modificados por usuario')}recordAudit('Datos del jugador modificados',p.id,'Revisión solicitada');savePlayers();saveInscriptions()}else{const pid=uid('p'),iid=uid('i');players.push({id:pid,name:full,birth:birth||'',dni:fd.get('dni')||'',data:true,active:true});inscriptions.push({id:iid,playerId:pid,seasonId:currentSeasonId,categoryId:cat?.id||'cat-pb',docs:'Pendiente de documentos',workflow:'review',federation:'Pendiente de revisión',status:'pending',familyActionRequired:false,returnMessage:'',createdAt:isoToday()});representations.push({id:uid('r'),playerId:pid,userId:currentUser.id,type:'guardian',startDate:isoToday(),endDate:'',reason:'Inscripción inicial',legalException:false});addStatusHistory(iid,pid,'','review','Inscripción creada por familia');recordAudit('Alta de jugador',pid,'Inscripción inicial');savePlayers();saveInscriptions();saveReps()}renderFamily();e.currentTarget.reset();editingPlayerId=null;$('#playerModal').close()};$$('#playerForm [value="cancel"]').forEach(b=>b.onclick=()=>{$('#playerForm').reset();editingPlayerId=null;$('#playerModal').close()});
...
$$('[data-club-view]').forEach(b=>b.onclick=()=>{if(currentRole==='coach'&&b.dataset.clubView!=='clubView')return;if(b.dataset.clubView==='usersView'&&currentRole!=='admin')return;setView(b.dataset.clubView);if(b.dataset.clubView==='clubView')renderClub();if(b.dataset.clubView==='medicalView')renderMedical();if(b.dataset.clubView==='coachesView')renderCoaches();if(b.dataset.clubView==='structureView')renderStructure();if(b.dataset.clubView==='usersView')renderClubUsers()});
