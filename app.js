...
function renderStructure(){
  ...
  if(dbStructureLoaded){
    ...
    $('#structureTeamsTable').innerHTML=dbSeasonTeams().map(t=>`<tr><td><strong>${esc(t.name)}</strong>${t.modality?`<div class="meta">${esc(t.modality)}</div>`:''}</td><td>${esc(dbCategoryName(t.categoryId))}</td><td>—</td><td>—</td><td><span class="status ${t.active?'complete':'returned'}">${t.active?'Activo':'Inactivo'}</span></td><td><span class="meta">Supabase</span></td></tr>`).join('');
    return;
  }
